...
# infile <- "https://dap.tern.org.au/thredds/fileServer/Historical_Extreme_Weather_Events_Australia/AWAP_Year/uncompressed/Un_AWAP_2021.nc"
# download.file(infile, "data_downloaded/Un_AWAP_2021.nc")
# infile <- "data_downloaded/Un_AWAP_2021.nc"
#### variables ####
...
sa1_out <- list()
for(var_i in c("Rain",       "Temp_max" ,  "Temp_min",   "VapPress09", "VapPress15")[-1]){
  #var_i <- "Temp_max"
  print(var_i)
for(yy in 2001:2020){
## yy = 2001
  print(yy)
study_period <- list(mindate=paste0(yy,"-01-01"), maxdate =paste0(yy,"-01-02"))

## raster data for each var
#print(var_i)
b <- raster::brick(gsub("Un_AWAP_2020.nc", paste0("Un_AWAP_",yy,".nc"), infile), varname = var_i)
# b
# plot(b)
...
sa1_out2
## check a day
qc <- sa1_out2[substr(sa1_out2$variable, 6, 11) == ".01.01"
         & SA1_MAIN16 == 60101100101 ]
qc
dcast(qc, SA1_MAIN16 +  ID  +  variable ~ measure )
